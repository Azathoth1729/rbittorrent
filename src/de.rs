            ...
            while remainder.chars().next() != Some('e') {
                ...
                if let (BencodeValue::Bytes(key), rest) = decoded {
                    let (value, rest) = decode_bencoded_value(rest)
                        .with_context(|| format!("Can't decoded when parsed value of map, str: {}\nprev key is: {:?}", rest, key))?;
...
fn decode_bencoded_int(encoded_int: &str) -> anyhow::Result<(BencodeValue, &str)> {
    encoded_int.strip_prefix('i').with_context(|| format!("str: {} can't strip prefix `i`", encoded_int))?
        ...
        .and_then(|(int_str, rest)| {
            ...
            let int = int_str.parse::<i64>().with_context(|| format!("Can't parse str : {} before `:` delimiter which should be a usize", &int_str))?;
